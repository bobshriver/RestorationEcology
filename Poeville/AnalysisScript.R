...
MessyData<-read.csv('PoevilleData2025.csv') ##This function will read in our dataset
MessyData<-MessyData[1:306,c(1:4)]
...
for(r in 1:nrow(MessyData)){
  ...
  data[samp,tax] <- as.numeric(MessyData[r, 2])
...
points(poe.mds, display = "sites", pch = c(18, 9) [as.numeric(rep(c(1,2),each=9))], col = c("green", "black") [rep(c(1,2),each=9)]) # displays site points wher
legend(-2,-.5, legend = c('Unburned',"Burned,Seeded"), pch = c(18, 9), col = c("green", "black"), bty = "n", cex = 1) #
if (ellipse==T) {ordiellipse(poe.mds, groups = as.numeric(rep(c(1,2),each=9)), draw = "polygon", lty = 1, col = "grey90")}
}
